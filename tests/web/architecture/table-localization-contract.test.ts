...
import { isUiMessageKey, translate } from "@src/features/campaign/i18n-core";
...
// The rendered feature tests live in the repository test tree (mirrored path),
// not next to the source they exercise.
const testRoot = resolve(process.cwd(), "../../tests/web");
...
describe("all web table localization contracts", () => {
  it("keeps a rendered test and a locale-aware header contract for every table", () => {
    for (const contract of tables) {
      ...
      expect(existsSync(resolve(testRoot, contract.test)), `${contract.source} needs a rendered test`).toBe(true);
