...
function processQueue(document:CampaignDocument,reader:CatalogueReader,post:NonNullable<ReturnType<typeof pendingFollowup>["post"]>,followup:OpenPayload):CampaignDocument {
  const queue=[...((followup["queue"]??[]) as OpenPayload[])]; const messages=[...((followup["messages"]??[]) as string[])]; let gold=post.gold_delta??0; let shards=post.wyrdstone_delta??0; let inventory=[...document.campaign.inventory]; let warriors=[...document.campaign.warriors]; let specialRules=[...document.campaign.special_rules]; let uniqueRewardIds=[...(document.campaign.unique_reward_ids??[])]; let current={...followup};
  ...
  while(queue.length) {
    ...
    if(kind==="grant_compound_item") { const id=String(node["reward_id"]??""); const name=String(node["name"]??id); const existing=inventory.find((row)=>row.id===id); inventory=existing?inventory.map((row)=>row.id===id?{...row,owned:row.owned+1,stash:row.stash+1}:row):[...inventory,{id,name,category:String(node["category"]??"Weapon"),owned:1,equipped:0,stash:1,value:0,special_rules:(node["rules"]??[]) as string[],base_item_id:String(node["base_item_id"]??"")}];messages.push(`+1 ${name} (stash)`);continue; }
    if(kind==="grant_special_item") { const id=String(node["item_id"]??""); const name=String(node["name"]??id); if(uniqueRewardIds.includes(id)){messages.push(`${name} already appeared; reward skipped`);continue;} const toStash=node["recipient"]==="stash"; const target=actor("$hero_id"); if(!toStash&&!target){current={...current,queue,messages,pending:{kind:"choose_hero",label:String(node["label"]??"Choose a bearer"),continuation:node}};break;} const rules=String(node["text"]??"")?[String(node["text"])]:[]; inventory=[...inventory,{id,name,category:"Magical Artefact",owned:1,equipped:toStash?0:1,stash:toStash?1:0,value:0,rarity:"Unique",special_rules:rules}]; if(target&&!toStash)warriors=warriors.map((row)=>row.id===target.id?{...row,equipment:[...row.equipment,{item_id:id,name,quantity:1,acquisition:"scenario_reward",unit_cost:0,per_model:false,transferable:true,special_rules:rules}]}:row);uniqueRewardIds.push(id);messages.push(toStash?`${name} kept in stash`:`${target!.name} receives ${name}`);continue; }
  ...
  return withCampaign(document,{...document.campaign,inventory,warriors,special_rules:specialRules,unique_reward_ids:uniqueRewardIds,post_battles:document.campaign.post_battles.map((row)=>row===post?changedPost:row)});
}
