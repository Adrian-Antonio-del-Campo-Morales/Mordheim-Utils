...
function replace(document:CampaignDocument,post:CampaignDocument["campaign"]["post_battles"][number],follow:OpenPayload,replacements:readonly OpenPayload[],description:string,dice:readonly number[]=[],roll?:number){const key=`${String(follow["warrior_id"])}:${Number(follow["casualty_index"]??1)}`,injuries={...((post.step_state?.["injuries"]??{}) as OpenPayload)},current={...((injuries[key]??{}) as OpenPayload)},history=Array.isArray(current["follow_up_rolls"])?current["follow_up_rolls"] as OpenPayload[]:[],step_state={...(post.step_state??{}),injuries:{...injuries,[key]:{...current,...(dice.length?{follow_up_rolls:[...history,{phase:String(follow["resolution_phase"]??"follow-up"),dice,roll}]}:{})}}},changed={...post,step_state,pending_follow_ups:(post.pending_follow_ups??[]).flatMap((row)=>row["id"]===follow["id"]?[...replacements]:[row]),event_log:[...(post.event_log??[]),{step:0,type:"injury_followup",warrior_id:follow["warrior_id"],description}]};return withCampaign(document,{...document.campaign,post_battles:document.campaign.post_battles.map((row)=>row.battle_number===post.battle_number?changed:row)});}

export function resolveInjuryTableFollowUp(document:CampaignDocument,reader:KnowledgeReader,input:{follow_up_id:string;roll:number;dice?:readonly number[]}):Result{
  ...
  const dice=injuryFollowUpDice(reader,follow),roll=Math.trunc(input.roll),rolled=input.dice??[];if(!dice||!valid(roll,dice)||rolled.length>0&&(rolled.length!==dice[0]||rolled.some((value)=>!Number.isInteger(value)||value<1||value>dice[1])||(dice[0]===2&&dice[1]===6?rolled[0]*10+rolled[1]:rolled.reduce((sum,value)=>sum+value,0))!==roll))return{ok:false,message:"Use a valid result for this injury roll."};
  ...
  if(phase==="effect_roll"){
    ...
    const changed=applied.document.campaign.post_battles.find((row)=>row.battle_number===post.battle_number)!;return{ok:true,document:replace(applied.document,changed,follow,[],`${warrior.name} misses ${roll} game(s).`,rolled,roll)};
  }
  if(phase==="repeat_count"){
    ...
    return{ok:true,document:replace(document,post,follow,repeats,`${warrior.name}: ${roll} additional serious injury roll(s) required.`,rolled,roll)};
  }
  ...
  const changed=applied.document.campaign.post_battles.find((row)=>row.battle_number===post.battle_number)!;return{ok:true,document:replace(applied.document,changed,follow,[],`${warrior.name}: ${String(outcome["result"]??"injury resolved")}`,rolled,roll)};
}
