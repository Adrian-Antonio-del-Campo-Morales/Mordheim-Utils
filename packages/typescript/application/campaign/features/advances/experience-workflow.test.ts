...
function makePending(xpDelta: number, warriors: readonly Warrior[], absentees: readonly unknown[] = []): CampaignDocument {
  const doc: unknown = {
    view: {},
    campaign: {
      ...
      warriors: [...warriors],
      battles: [{ number: 1, scenario_id: "scenario.skirmish", scenario_name: "Skirmish", opponent: "X", result: "Draw", xp_delta: xpDelta, absentees, xp_awards: {}, out_of_action_ids: [] }],
...
describe("applyBattleExperience (desktop apply_battle_experience)", () => {
  it("awards XP once and seeds a pending advance when a threshold is crossed", () => {
    ...
    // Henchman 5+4=9 crosses the 8-XP henchman threshold → an advance is seeded.
    const advances = result.document.campaign.post_battles[0].pending_advances!;
    expect(advances.some((row) => row["warrior_id"] === "hench-1" && row["threshold"] === 8)).toBe(true);
