...
export function correctResource(document:CampaignDocument,input:{resource:string;delta:number;reason:string}):Result {
  ...
  const changed={...post!,gold_delta:(post!.gold_delta??0)+(resource==="gold_crowns"?delta:0),wyrdstone_delta:(post!.wyrdstone_delta??0)+(resource==="wyrdstone_fragments"?delta:0),event_log:[...(post!.event_log??[]),{step:post!.active_step,type:"manual_resource_correction",description:`${resource}: ${delta>=0?"+":""}${delta} (${reason})`,resource,delta,reason}]};
...
export function addManualStashItem(document:CampaignDocument,reader:KnowledgeReader,input:{item_id:string;quantity:number;reason:string}):Result {
  ...
  const log={type:"manual_item_correction",item_id:input.item_id,quantity,reason}; const changed=post?{...post,event_log:[...(post.event_log??[]),{step:post.active_step,type:"manual_item_correction",description:`+${quantity} ${name} (${reason})`,item_id:input.item_id,quantity,reason}]}:null;
