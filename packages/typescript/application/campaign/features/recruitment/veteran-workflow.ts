...
export function applyVeteranPool(document:CampaignDocument,dice:readonly number[]):Result {
  ...
  const pool=dice[0]+dice[1]; const changed={...post,veteran_pool:pool,step_state:{...(post.step_state??{}),veterans:{resolved:true,dice:[...dice],pool}},event_log:[...(post.event_log??[]),{step:5,type:"veteran_pool",description:`Veteran experience pool set to ${pool} XP.`,pool}]};
