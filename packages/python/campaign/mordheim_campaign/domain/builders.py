"""campaign.domain.builders: construction of initial campaign states.

Builds the draft and example application states from canonical KnowledgePort
profiles. This is the only part of the domain that receives the port: the
models themselves (``campaign.domain.models``) never do, so the web port can
substitute its own KB read model without touching the state model.
"""
...
def _stats(values: tuple[int, ...]) -> dict[str, int]:
    return dict(zip(("M", "WS", "BS", "S", "T", "W", "I", "A", "Ld"), values, strict=True))
...
def make_draft_state(
    ...
    *,
    ...
    from mordheim_campaign.domain.models import AppState

    return AppState(campaign=campaign, selected_moment="draft:0", draft_warrior_tab="hero")


def make_example_state(port: "KnowledgePort"):
