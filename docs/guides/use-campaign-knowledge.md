# Using the campaign knowledge base
...
[campaign catalogue](../knowledge/campaign-catalogue.md) (identity, ownership,
price policy, catalogue state) and [campaign modelling](../knowledge/campaign-modeling.md)
(YAML shapes).
...
`tests/python/knowledge/test_campaign_catalogs.py` (Scenarios section).

The historical `cost` values of the equipment lists were collated against the
Trading Post (see [campaign catalogue](../knowledge/campaign-catalogue.md)); regenerate the report with
