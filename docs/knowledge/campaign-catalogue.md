...
- [Using the campaign knowledge base](../guides/use-campaign-knowledge.md) — how to query and extend the
  catalogue from application code (responsibility boundary, data-ownership
  table, loader contract, scenario-reward schema).
- [Campaign modelling](campaign-modeling.md) — YAML shapes for
...
The collation tool `tools/knowledge/maintenance/price-collation.py` compares every `cost` value
...
run `python tools/knowledge/maintenance/price-collation.py`). A warband list is the creation/recruitment price list of its source;
...
committed sidecar `tools/knowledge/maintenance/price-collation-resolutions.csv` holds 55 rows
...
`validate` gate and in `tests/python/knowledge/test_editorial_schemas.py`, before the
...
vocabulary — this document and [campaign modelling](campaign-modeling.md)
...
reference resolution) is covered by `tests/python/knowledge/test_campaign_loaders.py`.
...
[the component reference](../reference/components.md#warband-manager)).
