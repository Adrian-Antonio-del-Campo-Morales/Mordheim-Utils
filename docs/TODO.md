...
- Sources of truth for pending work: `docs/knowledge/hirelings.md` (the 59
  pending hireling references), `docs/knowledge/campaign-catalogue.md` (price-collation
...
- NumPy and native engines are candidate implementations: rebuild and certify
  ...
  design decisions live in [Design rulings](decisions/design-rulings.md).
...
- [ ] **Close the 28 pending semantic specifications.** `verify` currently
  ...
  - [ ] **Template plan** — do not hand-write 28 specifications from
      ...
      `python -m pytest tests/python/verification/test_semantics.py -q` and
...
`docs/knowledge/hirelings.md` documents **59 pending intrinsic references**
...
- [ ] The 4 deliberately out-of-scope Dramatis Personae profiles (composite
      ...
      `docs/knowledge/hirelings.md` until a schema for composite entities
      exists.
...
- [ ] **Price collation review queue** — the runs so far confirmed 18
      ...
      (committed sidecar `tools/knowledge/maintenance/price-collation-resolutions.csv`, 23 rows
      ...
      `python tools/knowledge/maintenance/price-collation.py` (writes `outputs/knowledge/`,
...
- [x] Band `name_i18n.es` / `effect_i18n.es` translation complete: 81/81
      warbands at 100% (`python tools/knowledge/maintenance/band_translation_status.py`).

Locale policy and the sanctioned i18n readers:
[the KB guide](reference/knowledge-base.md) and
