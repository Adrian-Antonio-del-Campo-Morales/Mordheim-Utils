...
class AddWarriorDialog(tk.Toplevel):
    ...
    def __init__(self, parent: tk.Misc, controller: AppController, *, kind: str) -> None:
        ...
        self.title(tr('Add Hero') if kind == "hero" else tr('Add Henchmen Group'))
        ...
        heading = tr('ADD HERO') if kind == "hero" else tr('ADD HENCHMEN GROUP')
        subtitle = tr('Choose a canonical profile from the warband roster.') if kind == "hero" else (
            tr('Choose a group profile; the whole group shares one profile card.')
        )
    ...
    def _build_picker(self, body: tk.Frame) -> None:
        ...
        for profile in self.profiles:
            self.listbox.insert("end", tr('{}  ·  {} gc  ·  XP {}').format(profile.name, profile.cost, profile.experience))
        if not self.profiles:
            self.listbox.insert("end", tr('(no profiles available)'))
    ...
    def _build_actions(self, body: tk.Frame) -> None:
        ...
        label = tr('ADD TO DRAFT') if self.profiles else tr('CLOSE')
    ...
    def _update_detail(self, _event=None) -> None:
        ...
        if profile is None:
            ...
            return
        taken, member_max = self.controller.profile_allowance(profile)
        allowance = tr('no roster limit') if member_max is None else tr('{} remaining').format(member_max - taken)
        lines = [
            "  ·  ".join(f"{key} {value}" for key, value in profile.characteristics.items()),
            tr('Cost {} gc per model  ·  Starting XP {}  ·  {}').format(profile.cost, profile.experience, allowance),
        ]
        if profile.skill_tables:
            lines.append(tr('Skill access: ') + ", ".join(profile.skill_tables))
        if profile.inherent_rules:
            lines.append(tr('Rules: ') + ", ".join(profile.inherent_rules))
        self.detail_var.set("\n".join(lines))
