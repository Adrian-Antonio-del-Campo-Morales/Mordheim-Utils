...
class RecordBattleDialog(tk.Toplevel):
    ...
    def __init__(self, parent: tk.Misc, controller: AppController) -> None:
        ...
        tk.Label(body, text=tr('RECORD BATTLE'), bg=COLORS["panel"], fg=COLORS["text"], font=("Georgia", 15)).pack(anchor="w")
        tk.Label(
            body,
    ...
    def _build_counters(self, body: tk.Frame) -> None:
        row = self._row(body, tr('EXPERIENCE'))
        self.xp_var = tk.IntVar(value=1)
        ttk.Spinbox(row, from_=0, to=99, width=5, textvariable=self.xp_var).pack(side="left")
        tk.Label(row, text=tr('total XP granted to each surviving warrior'), bg=COLORS["panel"], fg=COLORS["muted_dark"], font=("Segoe UI", 7)).pack(side="left", padx=(8, 0))
    ...
    def _build_actions(self, body: tk.Frame) -> None:
        ...
        ttk.Button(actions, text=tr('RECORD & RESOLVE'), style="Accent.TButton", command=self._record).pack(side="right")
    ...
    def _record(self) -> None:
        ...
        if not ok:
            ...
            messagebox.showerror(tr('Cannot record battle'), message, parent=messagebox_parent)
            return
        self.destroy()
