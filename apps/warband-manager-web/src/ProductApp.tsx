...
export function ProductApp() {
  ...
  const create = async () => { if (!knowledge) return; const band=bands.find((row)=>String(row.id)===bandId); if(!band)return; try { const service=createService(knowledge); const result=await service.createCampaign({ band_id: bandId, campaign_name: campaignName.trim() || (locale === "es" ? "Nueva Campaña de Mordheim" : "New Mordheim Campaign"), warband_name: resolveName(band, locale) }); if (result.ok) { addSession(service, "created"); setShowCreate(false); setCampaignName(""); } else setOperationError(result.message); } catch(error) { setOperationError(error instanceof Error ? error.message : String(error)); } };
  ...
  const exportSession = async (session: Session) => { try { const result=await session.service.prepareExport(); if (result.ok && result.payload) { download(result.payload.filename, result.payload.text, "application/json"); session.service.markExported(result.document); return true; } setOperationError(result.ok ? t.exportEmpty : result.message); } catch(error) { setOperationError(error instanceof Error ? error.message : String(error)); } return false; };
  const exportActive = async () => { if (active) await exportSession(active); };
  const undo = useCallback(async () => { if (!active) return; try { const result=await active.service.run("undo", {}); if (!result.ok) setOperationError(result.message); } catch(error) { setOperationError(error instanceof Error ? error.message : String(error)); } }, [active]);
  ...
  return <div className="app-shell">
    <header className="topbar"><button className="brand" onClick={() => setPage("campaign")}><strong>{locale === "es" ? "GESTOR DE CAMPAÑAS DE MORDHEIM" : "MORDHEIM CAMPAIGN MANAGER"}</strong><span>{locale === "es" ? "UNA BANDA A TRAVÉS DEL TIEMPO" : "A WARBAND THROUGH TIME"}</span></button>
      <div className="mobile-context" aria-live="polite" aria-label={`${active?.service.current()?.campaign.identity.warband_name ?? "Mordheim"}. ${active?.service.isDirty() ? t.unsaved : active ? (locale === "es" ? "Guardada" : "Saved") : t.empty}`} data-title={active?.service.current()?.campaign.identity.warband_name ?? "Mordheim"} data-status={active?.service.isDirty() ? t.unsaved : active ? (locale === "es" ? "Guardada" : "Saved") : t.empty} />
