...
export function ProductApp() {
  ...
  const exportActive = async () => { if (!active) return; const result=await active.service.prepareExport(); if (result.ok && result.payload) { download(result.payload.filename, result.payload.text, "application/json"); active.service.markExported(result.document); } };
  ...
  function SessionCards() { return <div className="session-grid">{sessions.map((session) => { const doc=session.service.current()!; return <article key={session.id} className={session.id===activeId ? "session-card active" : "session-card"}><span>{session.service.isDirty() ? t.unsaved : session.source}</span><h2>{doc.campaign.identity.warband_name}</h2><p>{doc.campaign.identity.campaign_name} · {doc.campaign.identity.warband_type}</p><div><button className="primary" onClick={() => { setActiveId(session.id); setPage("campaign"); }}>{t.open}</button><button onClick={() => { setSessions((all) => all.filter((item) => item.id!==session.id)); if(activeId===session.id) setActiveId(null); }}>{t.remove}</button></div></article>; })}</div>; }
}
