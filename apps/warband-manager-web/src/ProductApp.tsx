...
export function ProductApp() {
  ...
  useEffect(() => { if (operationError) { setShowCreate(false); setShowLibrary(false); setPendingRemove(null); setShowMore(false); } }, [operationError]);
  ...
  return <I18nProvider locale={locale}><div className="app-shell" onPointerDownCapture={(event) => { const control=(event.target as Element).closest<HTMLElement>("button:disabled, input:disabled, select:disabled, [aria-disabled=true]"); if(control)setOperationError(control.dataset.disabledReason || (locale === "es" ? "Esta acción no está disponible en el estado actual." : "This action is not available in the current state.")); }}>
    ...
    {operationError && <output className="global-error" role="alert">{localizeErrorMessage(operationError, locale, (id) => knowledgeName(knowledge ?? undefined, "profile", id, locale, id))} <button onClick={() => setOperationError(null)}>{t.dismiss}</button></output>}
    <main>
      {page==="campaign" && active && <CampaignAppProvider service={active.service} locale={locale} profileName={(id) => knowledgeName(knowledge ?? undefined, "profile", id, locale, id)}><Suspense fallback={null}><CampaignSlice knowledge={knowledge ?? undefined} locale={locale} /></Suspense></CampaignAppProvider>}
