...
function RarePurchase({ heroId, offer, document, locale }: { heroId: string; offer: ReturnType<typeof rareOffers>[number]; document: CampaignDocument; locale:"es"|"en" }) {
  ...
  if (offer.upgrade_multiplier) { const multiplier=offer.upgrade_multiplier; const weapons=document.campaign.inventory.filter((item)=>item.stash>0&&/weapon/i.test(item.category)); const selected=weapons.find((item)=>item.id===base); return <><label>{t.base}<select value={base} onChange={(event)=>setBase(event.target.value)}><option value="">{t.select}</option>{weapons.map((item)=><option value={item.id} key={item.id}>{item.name} · {(item.value??0)*multiplier} gc</option>)}</select></label><button className="primary" disabled={!selected} onClick={() => void app.runAction("upgradeRareSearch", { hero_id: heroId, base_item_id: base })}>{t.upgrade} {selected ? (selected.value??0) * multiplier : "?"} gc</button></> }
