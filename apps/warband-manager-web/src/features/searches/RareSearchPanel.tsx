...
export function RareSearchPanel({ document, knowledge, locale="en" }: { readonly document: CampaignDocument; readonly knowledge: ArtefactKnowledgeReader; readonly locale?:"es"|"en" }) {
  ...
  const rare = [...rareOffers(knowledge)].sort((a,b)=>a.name.localeCompare(b.name,locale));
  const dramatis = [...dramatisOffers(document, knowledge)].sort((a,b)=>a.name.localeCompare(b.name,locale));
