...
export function WarriorCard({ warrior, knowledge, locale, bandId }: { warrior: Warrior; knowledge?: ArtefactKnowledgeReader; locale: "es" | "en"; bandId?: string }) {
  ...
  return <article className="draft-warrior-card">
    <div className="draft-card-body">
      <section className="draft-card-identity"><header><div><strong>{displayName}</strong><span>{knowledgeName(knowledge, profileKind, warrior.profile_id, locale, warrior.profile_name)}</span></div><span className="draft-card-cost"><b>{warrior.kind === "hireling" ? `${locale === "es" ? "Contratación" : "Hiring fee"}: ` : ""}{warrior.quantity ?? 1} × {warrior.cost} gc</b>{warrior.kind === "hireling" && <small>{locale === "es" ? "Mantenimiento" : "Upkeep"}: {warrior.upkeep_resources?.length ? warrior.upkeep_resources.map(([resource, amount]) => resourceAmount(resource, amount, locale)).join(" + ") : "—"}</small>}</span></header><div className="stats">{Object.entries(warrior.stats).map(([key, value]) => <span key={key}><small>{localizedLabel(key, locale)}</small>{adaptCharacteristicValue(key, value, locale, warrior.stat_modifiers?.[key] ?? 0)}</span>)}</div></section>
