...
export function WarriorCard({ warrior, knowledge, locale }: { warrior: Warrior; knowledge?: ArtefactKnowledgeReader; locale: "es" | "en" }) {
  ...
  return <article className="draft-warrior-card">
    <div className="draft-card-body">
      ...
      <section className="draft-card-box"><h4>{locale === "es" ? "HABILIDADES / REGLAS" : "SKILLS / RULES"}</h4>{rules.map((entry) => <p key={entry.id}><KnowledgeHint knowledge={knowledge} kind={entry.kind} id={entry.id} locale={locale}>{entry.label}</KnowledgeHint></p>)}{rules.length === 0 && <p>—</p>}</section>
    </div>
