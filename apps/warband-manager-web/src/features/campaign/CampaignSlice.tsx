...
function PostBattleWorkspace({ document, knowledge, locale }: { document: CampaignDocument; knowledge?: ArtefactKnowledgeReader; locale: "es" | "en" }) {
  ...
  return <>
    ...
    <section aria-label={locale === "es" ? "Fase post-batalla" : "Post-battle phase"} className="post-battle-sequence"><header><div><span>{review ? (locale === "es" ? "SECUENCIA COMPLETA" : "SEQUENCE COMPLETE") : (locale === "es" ? `PASO ${Math.min(step + 1, 8)} DE 8` : `STEP ${Math.min(step + 1, 8)} OF 8`)}</span><h2>{review ? (locale === "es" ? "REVISIÓN FINAL" : "FINAL REVIEW") : labels[step]}</h2></div>{!review && step === post.active_step && step < 7 && <button className="primary post-battle-continue" type="button" onClick={next}>{locale === "es" ? `Continuar a ${labels[step + 1]}` : `Continue to ${labels[step + 1]}`}</button>}<b>{review ? (locale === "es" ? "8 ACCIONES COMPLETADAS" : "8 ACTIONS COMPLETE") : (locale === "es" ? `${7-post.active_step} ACCIONES RESTANTES` : `${7-post.active_step} ACTIONS REMAIN`)}</b></header><button type="button" className="mobile-steps-toggle" aria-expanded={showSteps} aria-controls="post-battle-steps" onClick={() => setShowSteps(!showSteps)}>{locale === "es" ? (showSteps ? "Ocultar pasos" : "Ver todos los pasos") : (showSteps ? "Hide steps" : "View all steps")}</button><div id="post-battle-steps" className={`post-battle-chapters ${showSteps ? "steps-expanded" : ""}`}>{chapters.map((chapter,index)=><div key={chapter}><strong>{chapter}</strong>{labels.slice(index*2,index*2+2).map((label,offset)=>{const position=index*2+offset;const complete=review || position<post.active_step;const active=!review && position===step;return <button type="button" key={label} className={active ? "active" : complete ? "complete" : ""} aria-current={active ? "step" : undefined} disabled={position>post.active_step} data-disabled-reason={position>post.active_step ? (locale === "es" ? "Completa primero los pasos anteriores." : "Complete the previous steps first.") : undefined} onClick={() => position<=post.active_step && selectStep(position)}>{complete ? "✓ " : ""}{label}</button>;})}</div>)}</div></section>
