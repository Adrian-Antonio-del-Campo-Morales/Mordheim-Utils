...
import { knowledgeName, readableValue, resourceAmount } from "./displayText";
...
function RosterOverview({ document, stateNumber, editable, locale, section, knowledge }: { document: CampaignDocument; stateNumber: number; editable: boolean; locale: "es" | "en"; section: "overview" | "warriors"; knowledge?: ArtefactKnowledgeReader }) {
  ...
  if ((section as string) === "warriors") return <section aria-label={locale === "es" ? "Guerreros" : "Warriors"}>{warriors.length === 0 ? <p>{t.missing}</p> : <div className="warrior-grid">{warriors.map((warrior) => <WarriorCard key={warrior.id} warrior={warrior} knowledge={knowledge} locale={locale} />)}</div>}</section>;
...
function PostBattleWorkspace({ document, knowledge, locale }: { document: CampaignDocument; knowledge?: ArtefactKnowledgeReader; locale: "es" | "en" }) {
  ...
  const selectStep = (index: number) => setSelectedStep(index);
  ...
  return <>
    ...
    {!review && step === 0 && <>{knowledge && <PostBattleInjuries document={document} knowledge={knowledge} locale={locale} />}<InjuriesPanel document={document} knowledge={knowledge} locale={locale} /><FollowUpAcknowledgements document={document} locale={locale} /></>}
    ...
    {!review && step === 6 && <>{knowledge && <RecruitProfilePanel document={document} knowledge={knowledge} locale={locale} />}<GroupRecruitmentPanel document={document} locale={locale} />{knowledge && <HirelingsPanel document={document} listings={knowledge} locale={locale} mode="hirelings" />}<HirelingUpkeepPanel document={document} locale={locale} /></>}
    {!review && step === 7 && <><>{knowledge && <HirelingsPanel document={document} listings={knowledge} locale={locale} mode="trading" />}<EquipmentPanel document={document} locale={locale} knowledge={knowledge} />{knowledge && <ManualCorrectionsPanel document={document} knowledge={knowledge} locale={locale} />}</>{step === post.active_step && <button type="button" className="primary review-button" onClick={() => setReviewOpen(true)}>{locale === "es" ? "ABRIR REVISIÓN FINAL" : "OPEN FINAL REVIEW"}</button>}</>}
    {review && <ReviewPanel document={document} locale={locale} knowledge={knowledge} />}
  </>;
}
