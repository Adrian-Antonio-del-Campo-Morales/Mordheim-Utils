...
export function VeteranPoolPanel({document,locale="en"}:{readonly document:CampaignDocument;readonly locale?:"es"|"en"}) {
  ...
  return <section aria-label={t.title}><h3>05 · {t.title}</h3>{resolved?<p role="status">{t.resolved}: {post.veteran_pool??0} XP. {t.shared}</p>:!post.sale_resolved?<p role="status">{t.first}</p>:<><p>{t.roll}</p><DiceResolver count={2} sides={6} label={t.dice} onResolve={(dice)=>void app.runAction("applyVeteranPool",{dice})}/></>}</section>;
}
