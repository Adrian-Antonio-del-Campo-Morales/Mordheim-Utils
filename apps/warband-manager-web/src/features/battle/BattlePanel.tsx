...
export function BattlePanel({ document, knowledge, locale = "en" }: BattlePanelProps) {
  ...
  const record = async () => { setBusy(true); await app.runAction("recordBattle", { scenario: scenario || String(scenarios[0]?.id ?? "unknown"), opponent, result, gold_delta: Number(gold) || 0, wyrdstone: Number(wyrdstone) || 0, xp_delta: Number(xp) || 0, out_of_action_ids: outOfAction, notes: notes.trim() }); setBusy(false); };
