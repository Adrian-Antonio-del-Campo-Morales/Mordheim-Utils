...
export function BattleHistory({ battle, locale, knowledge }: { readonly battle: Battle | undefined; readonly locale: "es" | "en"; readonly knowledge?: ArtefactKnowledgeReader }) {
  ...
  return <section className="page" aria-label={`${t.battle} #${battle.number}`}>
    <div className="page-title"><h2>{t.battle} #{battle.number}</h2><span>{knowledgeName(knowledge, "scenario", battle.scenario, locale, battle.scenario)} · {t.opponent}: {battle.opponent} · {battle.date}</span></div>
    ...
    {section === "overview" && <><dl className="campaign-metrics"><div><dt>{t.result}</dt><dd>{readableValue(battle.result, locale)}</dd></div><div><dt>{t.rating}</dt><dd>{battle.rating_before}</dd></div><div><dt>{t.opponentRating}</dt><dd>{battle.opponent_rating ?? "—"}</dd></div><div><dt>{t.deployed}</dt><dd>{battle.models_before}</dd></div></dl><article className="rule-detail"><h3>{t.casualties}</h3><p>{battle.casualties} {t.ooa}</p></article><article className="rule-detail"><h3>{t.consequences}</h3><ul><li>{t.xp}: +{battle.xp_delta}</li><li>{t.gold}: {battle.gold_delta} gc</li><li>{t.wyrdstone}: {battle.wyrdstone}</li><li>{t.advances}: {battle.advances}</li><li>{t.models}: {battle.models_before} → {battle.models_after}</li><li>{t.rating}: {battle.rating_before} → {battle.rating_after}</li></ul></article>{battle.scenario_results && <article className="rule-detail"><h3>{t.objectives}</h3>{objectives.length > 0 ? <ul>{objectives.map((line, index) => <li key={`${index}:${line}`}>{line}</li>)}</ul> : <p>—</p>}</article>}</>}
    {section === "participants" && <article className="rule-detail"><h3>{t.participants}</h3>{(battle.participants ?? []).map((participant) => { const id=String(participant.id ?? ""); const amount=Number(battle.per_group_casualties?.[id] ?? (ooa.has(id) ? 1 : 0)); return <p key={id}>{String(participant.name ?? readableValue(id, locale))}{Number(participant.quantity ?? 1)>1?` · ×${Number(participant.quantity)}`:""} — {amount>0?`${amount} ${t.ooa}`:t.participated}</p>; })}{(battle.absentees ?? []).map((participant) => <p className="condition" key={String(participant.id)}>{String(participant.name ?? readableValue(participant.id, locale))} — {t.absent} · {readableValue(participant.reason, locale)}</p>)}</article>}
    {section === "notes" && <article className="rule-detail"><h3>{t.notes}</h3><p>{battle.notes || t.noNotes}</p></article>}
  </section>;
}
