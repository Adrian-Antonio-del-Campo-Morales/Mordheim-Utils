"""Generate the desktop-to-web test traceability manifest.
...
It writes the ignored ``outputs/parity/test-manifests/`` artefact.

Determinism: output is byte-stable (sorted rows, no timestamps) so the
Python/TS gates can verify regeneration identity.

Usage:
    python tools/verification/make_test_manifest.py                # write manifest
    python tools/verification/make_test_manifest.py --stdout      # print instead
"""
...
MANIFEST_PATH = ROOT / "outputs" / "parity" / "test-manifests" / "campaign-test-manifest.json"
...
IMPLEMENTED_UI_TESTS: dict[tuple[str, str], str] = {
    ("tests/python/campaign/test_gui_interaction_regressions.py", test): "tests/web/architecture/product-app-session.test.tsx"
    for test in (
...
IMPLEMENTED_UI_TESTS.update({
    ("tests/python/campaign/test_gui_interaction_regressions.py", test):
        "tests/web/architecture/product-app-session.test.tsx"
    for test in (
...
IMPLEMENTED_UI_TESTS[(
    "tests/python/campaign/test_gui_interaction_regressions.py",
    "test_dirty_includes_battle_draft_but_not_navigation",
)] = "tests/typescript/application/campaign/service.test.ts"
IMPLEMENTED_UI_TESTS.update({
    ("tests/python/campaign/test_variable_prices_and_restrictions.py", test):
        "tests/typescript/domain/campaign/kernel/hireling-hire.test.ts"
    for test in (
...
IMPLEMENTED_UI_TESTS.update({
    ("tests/python/campaign/test_gui_interaction_regressions.py", test):
        "tests/typescript/application/campaign/features/searches/search-workflow.test.ts"
    for test in (
...
IMPLEMENTED_UI_TESTS.update({
    ("tests/python/ui/test_ui_i18n.py", test):
...
IMPLEMENTED_UI_TESTS[(
    "tests/python/campaign/test_post_battle_engine.py",
    "test_hire_conditional_requires_acceptance_roll",
)] = "tests/typescript/domain/campaign/kernel/hireling-hire.test.ts"
IMPLEMENTED_UI_TESTS.update({
    ("tests/python/campaign/test_gui_interaction_regressions.py", test):
        "tests/web/features/economy/ManualCorrectionsPanel.test.tsx"
    for test in (
...
# tests/python/ui files are desktop Tkinter UI behaviour -> U, except the
...
TARGET_OVERRIDES = {
    ...
    "test_ui_i18n.py": "tests/web/features/campaign/ui_i18n.test.ts",
}


def _family(file: str) -> str:
    name = Path(file).name
    if file.startswith("tests/python/ui/"):
...
def _web_target(disposition: str, file: str) -> str | None:
    ...
    fam = _family(file)
    if disposition == "M":
        return f"tests/typescript/domain/campaign/{fam}.test.ts"
    if disposition == "U":
        return f"tests/web/features/campaign/{fam}.test.tsx"
    if disposition == "I":
        return f"tests/python/web/parity/{fam}_interop_test.py"
    if disposition == "S":
        return f"tests/fixtures/parity/vectors/{fam}.json"
    return None  # X
...
def collect_desktop_tests() -> list[tuple[str, str]]:
    ...
    result = subprocess.run(
        [sys.executable, "-m", "pytest", "tests/python/campaign", "tests/python/ui",
...
def build_rows() -> list[dict]:
    ...
    for file, test in collect_desktop_tests():
        ...
        if file.startswith("tests/python/ui/"):
            ...
            if rule is None:
                raise SystemExit(
                    ...
                    f"FAMILY_RULES in tools/verification/make_test_manifest.py"
                )
        ...
        follow_up = None
        if (file.startswith("tests/python/ui/") or basename == "test_gui_interaction_regressions.py") and disposition != "X":
            status = "partial"
            evidence = "tests/web/features/campaign/parity-gui-regressions.test.tsx"
...
def build_manifest() -> dict:
    ...
    return {
        ...
        "generated_by": "tools/verification/make_test_manifest.py",
        "deterministic": True,
        "sources": ["tests/python/campaign", "tests/python/ui"],
