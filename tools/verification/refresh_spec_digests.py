...
Usage::
    ...
    python tools/verification/refresh_spec_digests.py            # report + rewrite
    python tools/verification/refresh_spec_digests.py --check    # report only
...
PACKAGE_ROOTS = (
    Path(__file__).resolve().parents[2] / "packages" / "python" / "combat-engine",
    Path(__file__).resolve().parents[2] / "packages" / "python" / "roster-construction",
    Path(__file__).resolve().parents[2] / "packages" / "python" / "core",
)
...
SPECS_ROOT = Path(__file__).resolve().parent.parent.parent / "tests" / "specs" / "semantic"
