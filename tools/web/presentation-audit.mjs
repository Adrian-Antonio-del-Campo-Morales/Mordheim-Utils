...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  // The tool lives in tools/web; the surface it audits is the web application.
  const repoRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
  const root = resolve(repoRoot, "apps", "warband-manager-web");
  ...
  const output = resolve(repoRoot, `outputs/web-presentation/${deep ? "gui-text-audit-deep" : strict ? "gui-text-audit" : "presentation-audit"}.json`);
