"""English-review cross-check for the 2B staging tree.

Reads every band package under ``sources/2B/bands/mordheim`` and re-derives
each number (profile costs/xp/characteristics, equipment prices, roster
limits, numbers quoted in rule texts) from the band's cached source text in
``build/cache/2b-pdfs/text/<id>.txt`` (or the 300 dpi OCR dump for scanned
sources). It then classifies each check:

    OK         the number was found in the source text in context
    NOT_FOUND  the source text (OCR quality) does not contain the context
    MISMATCH   contradicting digits were found in context

The report is written to ``build/cache/2b-pdfs/review-report.md`` and summed
to stdout. This tool is **read-only**: it never edits packages, the manifest
or the active knowledge base. Manual verdicts are recorded in
``sources/2B/english-review.md``; ``NOT_FOUND`` is acceptable (weak OCR),
``MISMATCH`` must be resolved before a row is marked ``english-reviewed``.
"""
...
# Scanned sources: their packages already passed a full manual 300/600 dpi
# digit audit (see sources/2B/discrepancy-verdicts.md); the OCR text for them
# is the 300 dpi dump, and automated context matching is unreliable.
SCANNED_SOURCES = {
    "clan-angrund-kep": CACHE / "cak300-ocr.txt",
    "crooked-moon-kep": CACHE / "cmk300-ocr.txt",
    "slave-uprising-kep": CACHE / "suk300-ocr.txt",
}
PREVIOUSLY_AUDITED = set(SCANNED_SOURCES)
...
def norm_source(text: str) -> str:
    """OCR text -> uppercase alnum stream for context searches."""
    return re.sub(r"[^A-Z0-9]", "", text.upper())


def load_source_text(band_id: str) -> str:
    if band_id in SCANNED_SOURCES:
        path = SCANNED_SOURCES[band_id]
...
def profile_cost_contexts(src: str, name: str) -> list[str]:
    """Digit groups on lines of the form '<N> gold crowns to hire' — profile
    cost lines. Because extraction interleaves section headers with cost lines,
    fall back to 'any digit line' matches near the profile name. We approximate
    by collecting every N preceding a GOLDCROWNSTOHIRE marker (the source order
    of profiles is mirrored in the package) plus name-adjacent digits."""
    groups = context_for(src, "GOLDCROWNSTOHIRE", 0)  # keep side-effect free shape
    del groups
    # digits directly before each hire marker (lookbehind via find)
    out: list[str] = []
    for m in re.finditer(r"(\d+)GOLDCROWNSTOHIRE", src):
...
def check_number(value, contexts: list[str]) -> str:
    """OK if the value appears among nearby digit groups, MISMATCH if others
    appear but not it, NOT_FOUND if nothing was found nearby."""
    v = str(value)
    if not contexts:
        return "NOT_FOUND"
    if v in contexts:
...
def review_band(band_id: str) -> dict:
    """Run all checks for one band; returns a per-band result dict."""
    pkg = BANDS / band_id
    ...
    checks: list[tuple[str, str, str]] = []  # (kind, label, verdict)

    # roster -----------------------------------------------------------------
    roster = band.get("roster") or {}
    for key, needles in (
        ("minimum_models", ["MINIMUMOF", "MINIMUM", "MINIMUMF"]),
        ("maximum_models", ["MAXIMUMOF", "MAXIMUM", "MAYNEVEREXCEED", "NEVEREXCEED"]),
    ):
        val = roster.get(key)
        if val is None:
            continue
    ...
    if gold is not None:
        groups = [m.group(1) for m in re.finditer(r"(\d+)GOLDCROWNS", src)]
    ...
    for p in profiles.get("profiles") or []:
        ...
        if p.get("experience") is not None:
            groups = context_for(src, f"{name}STARTSWITH", 4) + context_for(
                src, f"{name}STARTSWITH", 4
            )
    ...
    for lst in equip.get("equipment_lists") or []:
        for item in lst.get("items") or []:
            ...
            if cost is None:
                continue
            words = re.sub(r"[^A-Z0-9 ]", "", item_id.replace("_", " ")).upper()
            needle = re.sub(r"[^A-Z0-9]", "", words)
            if not needle:
                continue
    ...
    return {
        ...
        "checks": checks,
        "summary": summary,
        "scanned": band_id in PREVIOUSLY_AUDITED,
    }
...
def main() -> int:
    ...
    lines = ["# 2B english-review cross-check", ""]
    tot = {"OK": 0, "NOT_FOUND": 0, "MISMATCH": 0}
    for res in results:
        for k, v in res["summary"].items():
            tot[k] += v
        lines.append(f"## {res['band_id']}{' (scanned; manually audited)' if res['scanned'] else ''}")
        lines.append("")
