...
class WarriorCard(BorderedFrame):
    def __init__(self, master: tk.Misc, warrior: WarriorVM, *, on_edit=None, **kwargs) -> None:
        ...
        label = warrior.profile_name if warrior.kind == "hero" else f"{warrior.profile_name} · {warrior.quantity} members"
        ...
        tk.Label(exp, text=f"EXP  {previous} → {warrior.experience}" if previous != warrior.experience else f"EXP  {warrior.experience}", bg=COLORS["panel_alt"], fg=COLORS["muted"], font=("Segoe UI Semibold", 8)).pack(anchor="w")
        ...
        tk.Label(equip, text="EQUIPMENT", bg=COLORS["panel_alt"], fg=COLORS["muted"], font=("Segoe UI Semibold", 8)).pack(anchor="w", pady=(0, 5))
        ...
        tk.Label(skills, text="SKILLS / RULES", bg=COLORS["panel_alt"], fg=COLORS["muted"], font=("Segoe UI Semibold", 8)).pack(anchor="w", pady=(0, 5))
        if warrior.skills:
